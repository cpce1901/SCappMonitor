...
def export_excel_task(sensor, vars, date1, date2):
    ...
    ws.append(["ID", detail_1, detail_2, detail_3, "Fecha"])  # Reemplaza con tus nombres de columna
    
    # Agrega datos a las filas
    for dato in datos:
        ws.append([dato[0], dato[1], dato[2], dato[3], dato[4]])  # Reemplaza con los nombres de tus campos
    
    # Guarda el archivo Excel
    excel_filename = 'datos.xlsx'
    excel_path = os.path.join('media', excel_filename)  # Asegúrate de que 'media' sea una carpeta válida en tu proyecto
    wb.save(excel_path)
